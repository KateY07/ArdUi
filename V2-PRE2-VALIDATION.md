...
上线后用户实际 `irm | iex` 暴露了 Windows PowerShell 5.1 将 UTF-8 BOM 解码成 `ï»¿` 的问题；原有安装测试显式按 UTF-8 读取本地脚本，遗漏了 HTTP 解码环节。安装入口现改为 ASCII 无 BOM，保留全部安装逻辑，安装提示使用英文；打包工具也强制 ASCII。修复后脚本 SHA-256 为 `70c16934d2378e9d1bad7413d57ba95812b68a2f9e0a4c183f030391da8866f6`。真实 PS5.1 只读获取线上原文，确认首字符为 `$`、首条为赋值语句且完整哈希一致；按用户要求未再次执行安装。客户端与 FRD 二进制均未变化。
