...
case "$stage" in /tmp/ardui-v2.pre3-*) ;; *) echo 'Unexpected staging path' >&2; exit 1;; esac
cd "$stage"
python3 - <<'PY'
import hashlib,json,pathlib
manifest=json.loads(pathlib.Path('manifest.json').read_text())
assert manifest['version']=='v2.pre3'
...
assert path.name==str(path)=='FRD-v1.pre8-win-x64.tar.xz'
...
backup=/var/backups/ardui-before-v2.pre3-$stamp
web=/var/www/f.visnova.cn/ardui
install -d -m 700 "$backup"
cp "$web/install.ps1" "$backup/install.ps1"
cp "$web/index.html" "$backup/index.html"
install -m 644 ArdUi-v2.pre3.exe "$web/ArdUi-v2.pre3.exe.next"
install -m 644 FRD-v1.pre8-win-x64.tar.xz "$web/FRD-v1.pre8-win-x64.tar.xz.next"
mv "$web/ArdUi-v2.pre3.exe.next" "$web/ArdUi-v2.pre3.exe"
mv "$web/FRD-v1.pre8-win-x64.tar.xz.next" "$web/FRD-v1.pre8-win-x64.tar.xz"
install -m 644 manifest.json "$web/manifest-v2.pre3.json"
...
echo "Published v2.pre3; rollback files: $backup"
sha256sum "$web/ArdUi-v2.pre3.exe" "$web/FRD-v1.pre8-win-x64.tar.xz"
